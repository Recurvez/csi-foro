...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($mensaje)) {
        $stmt = $link->prepare("INSERT INTO san_valentin (username, mensaje, validado) VALUES (?, ?, 0)");
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: eventosanvalentin.php?status=success");
                exit();
            } else {
                // Redirigir con un mensaje de error
                header("Location: eventosanvalentin.php?status=error&message=" . urlencode($stmt->error));
                exit();
            }
        } else {
            // Redirigir con un mensaje de error en la preparación de la consulta
            header("Location: eventosanvalentin.php?status=error&message=" . urlencode($link->error));
            exit();
        }
    } else {
        // Redirigir con un mensaje de advertencia si el mensaje está vacío
        header("Location: eventosanvalentin.php?status=warning&message=La carta no puede estar vacía.");
        exit();
    }
}

// Mostrar mensajes de éxito o error basados en los parámetros de la URL
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'success',
                    title: '¡Carta enviada!',
                    text: 'Tu carta ha sido enviada correctamente, será revisada por el equipo administrativo para ser publicada próximamente.',
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Hubo un error al enviar la carta: " . addslashes($_GET['message']) . "',
                });
            });
        </script>";
    } elseif ($_GET['status'] === 'warning' && isset($_GET['message'])) {
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'warning',
    ...
    <div class="section" id="section2">
        ...
        <div class="letter-container">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                ...
                <textarea id="letterText" name="letterText" maxlength="2048" placeholder="Lo sentimos, este evento está actualmente cerrado."></textarea>

                <button type="submit" hidden>Enviar Carta</button>
