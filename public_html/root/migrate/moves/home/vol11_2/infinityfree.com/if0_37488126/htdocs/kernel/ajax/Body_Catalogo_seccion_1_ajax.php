...
<?php
	...
	if($action == 'ajax'){
		...
		$count_query   = $link->query("SELECT count(*) AS numrows FROM furnis WHERE seccion = '1'");
		if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
		$total_pages = ceil($numrows/$per_page);
		$reload = 'furni.php';
		//consulta principal para recuperar los datos
		$query = $link->query("SELECT * FROM furnis WHERE seccion = '1' order by id DESC LIMIT $offset,$per_page");
		
		if ($numrows>0){
			while($row = mysqli_fetch_array($query)){
				?>
<div class="itemBox" style="background: url(<?php echo "$row[imagen]"; ?>) #353333 50% no-repeat;"><div class="itemName"><?php echo "$row[nombre]"; ?></div><div class="itemPrice">
<div class="icon"><img src="<?php echo "$row[icon]"; ?>"></div><span class="iconR"><?php echo "$row[creditos]"; ?></span></div></div>
				<?php
			}
			?>
