...
if ($action == 'ajax') {
    ...
    $whereClause = !empty($search) ? "WHERE user_logeado LIKE '%$search%'" : "";
    $count_query   = $link->query("SELECT count(*) AS numrows FROM logs_sospechosos $whereClause");
    ...
    $query = $link->query("SELECT * FROM logs_sospechosos $whereClause ORDER BY id DESC LIMIT $offset, $per_page");
