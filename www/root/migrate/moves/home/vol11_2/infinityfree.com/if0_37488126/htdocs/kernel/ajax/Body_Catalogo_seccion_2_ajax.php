...
<?php
	...
	if($action == 'ajax'){
		...
		$count_query   = $link->query("SELECT count(*) AS numrows FROM furnis WHERE seccion = '2'");
		if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
		$total_pages = ceil($numrows/$per_page);
		$reload = 'furni.php';
		//consulta principal para recuperar los datos
		$query = $link->query("SELECT * FROM furnis WHERE seccion = '2' order by id DESC LIMIT $offset,$per_page");
