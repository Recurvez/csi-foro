  ...
  $('.start-time').off().on('click', function () {
    ...
    if (!username || !encargado) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Falta el nombre de usuario o el encargado.'
      });
      return;
    }
...
function verificarNuevaAccion() {
  $.getJSON('times/get_ultima_accion.php', function (data) {
    if (data.fecha !== ultimaFechaMostrada) {
      ...
      tabla.ajax.reload(null, false); // AQUÍ es donde sí recargas solo cuando alguien hizo una acción
    }
  });
}
...
setInterval(actualizarContadoresDescendentes, 1000); // Esto sí mantiene los timers funcionando normal
setInterval(verificarNuevaAccion, 3000); // Revisa cada 3 seg si hay nuevas acciones
