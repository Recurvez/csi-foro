...
$(document).ready(function(){
	$('#baseimg').css('background-image', 'url(utilities/utilidad-grupos/sistemas/placas/base/'+basfinal+'.gif)');
	for(i=0;i<objetos.length;i++){
		if(objetos[i]['desac'] == true)
			$('#accimg-'+i).css('background-image', 'url(utilities/utilidad-grupos/img/cancelar.gif)');
		else
			$('#accimg-'+i).css('background-image', 'url(utilities/utilidad-grupos/sistemas/placas/templates/'+objetos[i]['result']+'.gif)');
		colorBase(cbase);
	...
	$('div.imgprev.obj').click(function(){
		...
		if(objetos[op]['desac'] == false) {
			$(this).css('background-image', 'url(utilities/utilidad-grupos/img/cancelar.gif)');
			objetos[op]['desac'] = true;
		}else{
			$(this).css('background-image', 'url(utilities/utilidad-grupos/sistemas/placas/templates/'+objetos[op]['result']+'.gif)');
...
function cambiarObjeto(accion, id) {
	...
	$('#accimg-'+id).css('background-image', 'url(utilities/utilidad-grupos/sistemas/placas/templates/'+objetos[id]['result']+'.gif)');
...
function verlink() {
	...
	$('#placafinal').attr('src','utilities/utilidad-grupos/sistemas/badge.php?placa='+milink); 
}